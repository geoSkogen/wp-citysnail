...
var monster_submits = [ document.querySelector('#submit'),
  document.querySelector('#top_submit')];
var monster_slugs = []
var index = -1
var mode = ''
...
var click_tasks = {
  ...
  'submit_click' : function () { update_mypages() }
}
...
for (var i = 0; i < monster_rows.length; i++) {
  ...
  assign_keypress(monster_fields[i],index)
  // NOTE - assign tab-key listener to toggle input open - open only!
}

for (var i = 0; i < monster_submits.length; i++ ) {
  index = i;
  assign_clicks('submit_click',monster_submits[i],index)
}
...
function assign_keypress(element,index) {
  element.addEventListener('keypress', function (event) {
    var code = event.keyCode.toString()
    if (keypress_tasks[code]) {
      'got keycode event ' + code
      keypress_tasks[code](element,index)
    }
  })
}
